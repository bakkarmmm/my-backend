...
export const getCtegories = async (req, res) => {
  try {
    ...
    const categories = await Category.find({ bussninsId: bussnise._id }).sort({
      order: 1,
    });
...
export const UpdateCategorie = async (req, res) => {
  ...
  try {
    ...
    console.log(id);
    if (!id) {
      res.status(404).json("this categorie not found");
    }
...
export const reorderCategories = async (req, res) => {
  ...
  try {
    ...
    const updates = categories.map((id, index) =>
      Category.findOneAndUpdate(
        { _id: id, bussninsId: bussnise._id },
        { order: index + 1 },
      ),
    );
